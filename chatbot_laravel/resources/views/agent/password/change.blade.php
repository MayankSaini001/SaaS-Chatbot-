...
@php
    $__routePrefix = auth()->user()->role === 'admin' ? 'admin' : 'agent';
@endphp
...
<div class="max-w-md mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border p-8 mb-6">
        <div class="text-center">
            ...
            <form method="POST" action="{{ route($__routePrefix.'.profile.avatar') }}" enctype="multipart/form-data" class="mt-5">
                @csrf
            ...
            @if(auth()->user()->avatar)
            <form method="POST" action="{{ route($__routePrefix.'.profile.avatar.remove') }}" class="mt-2"
    ...
    <div class="bg-white rounded-xl shadow-sm border p-8">
        ...
        <form method="POST" action="{{ route($__routePrefix.'.password.update') }}">
            @csrf
        ...
        <div class="mt-4 text-center">
            <a href="{{ route($__routePrefix.'.dashboard') }}" class="text-sm text-gray-400 hover:text-gray-600">← Back to Dashboard</a>
        </div>
    </div>
</div>
@endsection