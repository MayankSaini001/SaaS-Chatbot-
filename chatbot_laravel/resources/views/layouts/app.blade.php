...
<div class="app-layout">
    ...
    <aside class="app-sidebar" id="sidebar">
        ...
        <a href="{{ route('agent.auto-replies') }}"
           class="sb-link {{ request()->routeIs('agent.auto-replies*') ? 'active' : '' }}">
            <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <a href="{{ route('agent.password.change') }}" class="sb-user" style="text-decoration:none;">
            <div class="sb-user-avatar" style="overflow:hidden;position:relative;">
                @if(auth()->user()->avatar)
                    <img src="{{ auth()->user()->avatar }}" alt="{{ auth()->user()->name }}" style="width:100%;height:100%;object-fit:cover;">
                @else
                    {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
                @endif
                <span id="sb-status-dot" style="position:absolute;bottom:-1px;right:-1px;width:10px;height:10px;border-radius:50%;border:2px solid #1e1b4b;background:#22c55e;"></span>
            </div>
        ...
        <div style="padding:0 20px 16px;">
            <div id="sb-status-toggle" style="display:flex;gap:4px;background:rgba(255,255,255,0.04);border-radius:10px;padding:3px;">
                <button type="button" class="sb-status-btn" data-status="online" title="Online — visible to visitors" style="flex:1;font-size:10px;font-weight:600;padding:6px 4px;border-radius:8px;border:none;cursor:pointer;background:none;color:#94a3b8;">🟢 Online</button>
                <button type="button" class="sb-status-btn" data-status="away" title="Away — hidden from visitors" style="flex:1;font-size:10px;font-weight:600;padding:6px 4px;border-radius:8px;border:none;cursor:pointer;background:none;color:#94a3b8;">🟡 Away</button>
                <button type="button" class="sb-status-btn" data-status="busy" title="Busy — hidden from visitors" style="flex:1;font-size:10px;font-weight:600;padding:6px 4px;border-radius:8px;border:none;cursor:pointer;background:none;color:#94a3b8;">🔴 Busy</button>
            </div>
        </div>

        <script>
        (function() {
            var CURRENT_STATUS = '{{ auth()->user()->status ?? 'online' }}';
            var dotColors = { online: '#22c55e', away: '#f59e0b', busy: '#ef4444' };
            var dot = document.getElementById('sb-status-dot');
            var btns = document.querySelectorAll('.sb-status-btn');

            function paint(status) {
                btns.forEach(function(b) {
                    var active = b.getAttribute('data-status') === status;
                    b.style.background = active ? 'rgba(255,255,255,0.12)' : 'none';
                    b.style.color = active ? '#fff' : '#94a3b8';
                });
                if (dot) dot.style.background = dotColors[status] || dotColors.online;
            }

            paint(CURRENT_STATUS);
            ...
            btns.forEach(function(btn) {
                btn.addEventListener('click', function(e) {
                    ...
                    paint(status);
                    fetch("{{ route('agent.status.update') }}", {
                        method: 'POST',
                        headers: {
                    ...
                    .catch(function() { paint(CURRENT_STATUS); });
                });
            });
        })();
        </script>
...
(function () {
    ...
    agentChannel.bind('note.mentioned', function (data) {
        playSound();
        showDesktopNotification(
            (data.authorName || 'Someone') + ' mentioned you in a note',
            (data.visitorName ? '(' + data.visitorName + ') ' : '') + (data.snippet || '')
        );
    });
