...
Route::prefix('agent')
    ...
    ->group(function () {
        ...
        Route::get('/auto-replies', [\App\Http\Controllers\Agent\AutoReplyController::class, 'index'])
            ->name('agent.auto-replies');

        Route::post('/auto-replies', [\App\Http\Controllers\Agent\AutoReplyController::class, 'store'])
            ->middleware('not_viewer')
            ->name('agent.auto-replies.store');

        Route::post('/auto-replies/{autoReply}/toggle', [\App\Http\Controllers\Agent\AutoReplyController::class, 'toggle'])
            ->middleware('not_viewer')
            ->name('agent.auto-replies.toggle');

        Route::delete('/auto-replies/{autoReply}', [\App\Http\Controllers\Agent\AutoReplyController::class, 'destroy'])
            ->middleware('not_viewer')
            ->name('agent.auto-replies.destroy');
        ...
        Route::post('/conversations/{conversation}/mark-read', [ConversationController::class, 'markRead'])
            ->name('agent.conversations.markRead');
		...
		Route::post('/status', [ConversationController::class, 'updateStatus'])
