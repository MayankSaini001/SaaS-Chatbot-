(function () {
  ...
  margin-bottom:3px;
    ...
    .cb-seen-label {
      font-size: 10px; color: #8b5cf6; text-align: right;
...
channel.bind('message.sent', function(data) {
    ...
    channel.bind('messages.read', function(data) {
      var wraps = msgs.querySelectorAll('.cb-msg-v-wrap');
      if (!wraps.length) return;
      // Agent ne "abhi tak ke" saare visitor messages padh liye — sabpe
      // data-read set kar do, seen indicator sirf aakhri par dikhega.
      for (var i = 0; i < wraps.length; i++) wraps[i].setAttribute('data-read', '1');
    ...
    // Visitor was unblocked (or conversation otherwise reopened) — undo
    // whatever showResolved()/showBlockedState() did: bring back the
    // message input, re-arm polling, and remember the conversation again
    // so the chat can continue right where it left off.
    channel.bind('conversation.reopened', function() {
      if (!isResolved) return;
      isResolved = false;
      try { sessionStorage.setItem('cb_conversation_' + token, conversationId); } catch (e) {}
      restoreInput();
      startPolling();
    });
  }
  ...
  list.forEach(function(m) {
    ...
    if (m.sender_type === 'system') {
      ...
      if (m.is_deleted) {
        ...
        addVisitorBubble('', m.created_at, m.visitor_name, m.id, false, true, false, m.is_read);
        ...
        addVisitorBubble(
          ...
          m.created_at,
          m.visitor_name,
          m.id, m.is_edited, false, true, m.is_read
        );
        ...
        addVisitorBubble(
          ...
          m.created_at,
          m.visitor_name,
          m.id, m.is_edited, false, false, m.is_read
        );
...
function addVisitorBubble(text, time, name, id, isEdited, isDeleted, isImgMsg, isRead) {
  var isImg = isImgMsg || (typeof text === 'string' && /^\s*<img\s/i.test(text));
  var canEditDelete = !!id && !isImg && !isDeleted;
  var wrap = document.createElement('div');
  wrap.className = 'cb-msg-v-wrap';
  if (id) wrap.setAttribute('data-mid', id);
  wrap.setAttribute('data-read', isRead ? '1' : '0');
...
/**
 * Read Receipts — sirf sabse aakhri visitor bubble ke neeche "Seen"
 * dikhta hai (WhatsApp-style), agar us tak ke saare messages agent ne
 * padh liye hain (data-read="1"). Har render pass ke baad call karo.
 */
function refreshSeenIndicator() {
  var oldLabels = msgs.querySelectorAll('.cb-seen-label');
  for (var i = 0; i < oldLabels.length; i++) oldLabels[i].parentNode.removeChild(oldLabels[i]);

  var wraps = msgs.querySelectorAll('.cb-msg-v-wrap');
  if (!wraps.length) return;
  var last = wraps[wraps.length - 1];
  if (last.getAttribute('data-read') !== '1') return;

  var label = document.createElement('div');
  label.className = 'cb-seen-label';
  label.textContent = 'Seen';
  var timeEl = last.querySelector('.cb-time');
  if (timeEl) timeEl.insertAdjacentElement('afterend', label);
  else last.appendChild(label);
}
...
.then(function(data){
    ...
    // FAQ / Keyword Auto-Reply Bot — turant reply aa gaya, render kar do
    // (dedup ke liye lastMessageId already aage badha do taaki poll/pusher
    // isi message ko dubara na jode).
    if (data.auto_reply && data.auto_reply.id) {
        lastMessageId = Math.max(lastMessageId, data.auto_reply.id);
        if (!msgs.querySelector('[data-mid="' + data.auto_reply.id + '"]')) {
            addAgentBubble(data.auto_reply.body, new Date().toISOString(), 'Quick Reply', data.auto_reply.id, false);
        }
    }
